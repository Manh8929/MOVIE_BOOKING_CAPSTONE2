...
import { getUserBooking } from "../services/userService";
import {
  getAvailableComment,
  postReview,
  deleteReview,
...
const MovieDetail = () => {
  ...
  const [hasBookedConfirmedMovie, setHasBookedConfirmedMovie] = useState(false);
  ...
  //đặt phim mới được commnet
  useEffect(() => {
    const fetchUserBookings = async () => {
      try {
        const token = localStorage.getItem("token");
        if (!token || !userInfo?.user_id) return;
        const bookings = await getUserBooking(userInfo.user_id, token);

        if (!Array.isArray(bookings)) {
          console.error("Dữ liệu bookings không hợp lệ:", bookings);
          return;
        }
        ...
        const hasBooking = bookings.some((booking) => {
          return (
            booking.user_id === userInfo.user_id &&
            booking.status === "confirmed" &&
            booking.Showtime?.Movie?.movie_id === selectedMovieId
          );
        });

        setHasBookedConfirmedMovie(hasBooking);
      } catch (error) {
        console.error("Lỗi khi kiểm tra lịch sử đặt vé:", error);
      }
    };
  ...
  const handlePostComment = () => {
    ...
    if (!hasBookedConfirmedMovie) {
      toast.error("Bạn cần đặt vé xem phim này trước khi nhận xét!");
      return;
    }
  ...
  const negativeReviews = comments.filter(
